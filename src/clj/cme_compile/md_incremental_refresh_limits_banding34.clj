(ns cme-compile.md-incremental-refresh-limits-banding34
...
(defn- message-write [^RecordConsumer rconsumer #^long start-at ^MDIncrementalRefreshLimitsBanding34Decoder top-level]
  ...
  (let [repetition-decoder (.noMDEntries top-level)]
    (when (> (.count repetition-decoder) 0)
      ...
      (while (.hasNext ^java.util.Iterator repetition-decoder)
        (let [^MDIncrementalRefreshLimitsBanding34Decoder$NoMDEntriesDecoder repetition (.next ^java.util.Iterator repetition-decoder)]
          ...
          (with-field-new rconsumer "high_limit_price_exponent" 1 (.addInteger -7))

          (let [src-value (.mantissa (.lowLimitPrice repetition))]
            (when (not= src-value (. mktdata.PRICENULLDecoder mantissaNullValue))
              (with-field-new rconsumer "low_limit_price_mantissa" 2 (.addLong src-value))))

          (with-field-new rconsumer "low_limit_price_exponent" 3 (.addInteger -7))
          ...
          (with-field-new rconsumer "max_price_variation_exponent" 5 (.addInteger -7))
  ...
  (adapt-common (+ start-at 1) MDIncrementalRefreshLimitsBanding34Decoder))
(defn ^ParquetRecordWriter make-record-writer [prefix]
  (make-parquet-integrated-writer (str prefix "-md-incremental-refresh-limits-banding34.parquet")
...
(defn decode [subOffset actingBlockLength actingVersion buffer]
  (.wrap (new MDIncrementalRefreshLimitsBanding34Decoder) buffer subOffset actingBlockLength actingVersion))
