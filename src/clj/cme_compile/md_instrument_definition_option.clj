(ns cme-compile.md-instrument-definition-option
...
(with-field-new ~'rconsumer "put_or_call"  (+ ~start-at 16) (.addInteger (.value (.putOrCall ~'top-level))))

(let [src-value# (.year (.maturityMonthYear ~'top-level))]
 (when (not= src-value# (. ~mktdata.MaturityMonthYearDecoder yearNullValue))
(with-field-new ~'rconsumer "maturity_month_year_year"  (+ ~start-at 17) (.addInteger src-value#))))

		(let [src-value# (.month (.maturityMonthYear ~'top-level))]
 (when (not= src-value# (. ~mktdata.MaturityMonthYearDecoder monthNullValue))
(with-field-new ~'rconsumer "maturity_month_year_month"  (+ ~start-at 18) (.addInteger src-value#))))

		(let [src-value# (.day (.maturityMonthYear ~'top-level))]
 (when (not= src-value# (. ~mktdata.MaturityMonthYearDecoder dayNullValue))
(with-field-new ~'rconsumer "maturity_month_year_day"  (+ ~start-at 19) (.addInteger src-value#))))

		(let [src-value# (.week (.maturityMonthYear ~'top-level))]
 (when (not= src-value# (. ~mktdata.MaturityMonthYearDecoder weekNullValue))
(with-field-new ~'rconsumer "maturity_month_year_week"  (+ ~start-at 20) (.addInteger src-value#))))

(with-field-new ~'rconsumer "currency"  (+ ~start-at 21) (.addBinary (Binary/fromCharSequence (.currency ~'top-level))))

(with-field-new ~'rconsumer "strike_currency"  (+ ~start-at 22) (.addBinary (Binary/fromCharSequence (.strikeCurrency ~'top-level))))

(with-field-new ~'rconsumer "settl_currency"  (+ ~start-at 23) (.addBinary (Binary/fromCharSequence (.settlCurrency ~'top-level))))

(with-field-new ~'rconsumer "match_algorithm"  (+ ~start-at 24) (.addInteger (.matchAlgorithm ~'top-level)))

(with-field-new ~'rconsumer "min_trade_vol"  (+ ~start-at 25) (.addInteger (.minTradeVol ~'top-level)))

(with-field-new ~'rconsumer "max_trade_vol"  (+ ~start-at 26) (.addInteger (.maxTradeVol ~'top-level)))

(let [src-value# (.tickRule ~'top-level)]
 (when (not= src-value# (. ~decoder-type tickRuleNullValue))
(with-field-new ~'rconsumer "tick_rule"  (+ ~start-at 27) (.addInteger src-value#))))

(let [src-value# (.mainFraction ~'top-level)]
 (when (not= src-value# (. ~decoder-type mainFractionNullValue))
(with-field-new ~'rconsumer "main_fraction"  (+ ~start-at 28) (.addInteger src-value#))))

(let [src-value# (.subFraction ~'top-level)]
 (when (not= src-value# (. ~decoder-type subFractionNullValue))
(with-field-new ~'rconsumer "sub_fraction"  (+ ~start-at 29) (.addInteger src-value#))))

(let [src-value# (.priceDisplayFormat ~'top-level)]
 (when (not= src-value# (. ~decoder-type priceDisplayFormatNullValue))
(with-field-new ~'rconsumer "price_display_format"  (+ ~start-at 30) (.addInteger src-value#))))

(with-field-new ~'rconsumer "unit_of_measure"  (+ ~start-at 31) (.addBinary (Binary/fromCharSequence (.unitOfMeasure ~'top-level))))

(with-field-new ~'rconsumer "settl_price_type"  (+ ~start-at 32) (.addInteger (let [field# (.settlPriceType ~'top-level)] (.getByte (.buffer field#) (.offset field#)))))

(let [src-value# (.clearedVolume ~'top-level)]
 (when (not= src-value# (. ~decoder-type clearedVolumeNullValue))
(with-field-new ~'rconsumer "cleared_volume"  (+ ~start-at 33) (.addInteger src-value#))))

(let [src-value# (.openInterestQty ~'top-level)]
 (when (not= src-value# (. ~decoder-type openInterestQtyNullValue))
(with-field-new ~'rconsumer "open_interest_qty"  (+ ~start-at 34) (.addInteger src-value#))))

(with-field-new ~'rconsumer "user_defined_instrument"  (+ ~start-at 35) (.addInteger (Binary/fromCharSequence (.userDefinedInstrument ~'top-level))))

(let [src-value# (.tradingReferenceDate ~'top-level)]
 (when (not= src-value# (. ~decoder-type tradingReferenceDateNullValue))
(with-field-new ~'rconsumer "trading_reference_date"  (+ ~start-at 36) (.addInteger src-value#))))
...
(defmacro adapt-underlyings [decoder-type start-at] 
 (let [next-group (gensym)]
 `(do
(let [repetition-decoder# (.noUnderlyings ~'top-level)]
(when (> (.count repetition-decoder#) 0)
(.startField ~'rconsumer "underlyings" (+ ~start-at 0))
(while (.hasNext ^java.util.Iterator repetition-decoder#)
(let [~next-group (.next ^java.util.Iterator repetition-decoder#)
repetition# ~(with-meta next-group {:tag decoder-type})]

(.startGroup ~'rconsumer)
(with-field-new ~'rconsumer "underlying_security_id" 0 (.addInteger (.underlyingSecurityID repetition#)))

(with-field-new ~'rconsumer "underlying_security_id_source" 1 (.addInteger (Integer/parseInt \8)))

(with-field-new ~'rconsumer "underlying_symbol" 2 (.addBinary (Binary/fromCharSequence (.underlyingSymbol repetition#))))

(.endGroup ~'rconsumer)
)) (.endField ~'rconsumer "underlyings" (+ ~start-at 0))))

)))

(defmacro adapt-related-instruments [decoder-type start-at] 
 (let [next-group (gensym)]
 `(do
(let [repetition-decoder# (.noRelatedInstruments ~'top-level)]
(when (> (.count repetition-decoder#) 0)
(.startField ~'rconsumer "related_instruments" (+ ~start-at 0))
(while (.hasNext ^java.util.Iterator repetition-decoder#)
(let [~next-group (.next ^java.util.Iterator repetition-decoder#)
repetition# ~(with-meta next-group {:tag decoder-type})]

(.startGroup ~'rconsumer)
(with-field-new ~'rconsumer "related_security_id" 0 (.addInteger (.relatedSecurityID repetition#)))

(with-field-new ~'rconsumer "related_security_id_source" 1 (.addInteger (Integer/parseInt \8)))

(with-field-new ~'rconsumer "related_symbol" 2 (.addBinary (Binary/fromCharSequence (.relatedSymbol repetition#))))

(.endGroup ~'rconsumer)
)) (.endField ~'rconsumer "related_instruments" (+ ~start-at 0))))

)))


(def message-schema "  optional int64 strike_price_mantissa;
  optional int32 strike_price_exponent;
  optional int64 min_cab_price_mantissa;
  optional int32 min_cab_price_exponent;
  optional int64 min_price_increment_mantissa;
  ...
  repeated group underlyings {
    optional int32 underlying_security_id;
    optional int32 underlying_security_id_source;
    optional binary underlying_symbol;
  }
  repeated group related_instruments {
    optional int32 related_security_id;
    optional int32 related_security_id_source;
    optional binary related_symbol;
  }
