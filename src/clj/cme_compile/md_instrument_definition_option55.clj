  ...
  (ns cme-compile.md-instrument-definition-option55
  ...
  (defn- message-write [^RecordConsumer rconsumer #^long start-at ^MDInstrumentDefinitionOption55Decoder top-level]
...
(with-field-new rconsumer "strike_price_mantissa"  (+ start-at 0) (.addLong src-value))))
		...
		(with-field-new rconsumer "strike_price_exponent"  (+ start-at 1) (.addInteger -9))

(let [src-value (.mantissa (.minCabPrice top-level))]
 (when (not= src-value (. mktdata.PRICENULL9Decoder mantissaNullValue))
(with-field-new rconsumer "min_cab_price_mantissa"  (+ start-at 2) (.addLong src-value))))

		(with-field-new rconsumer "min_cab_price_exponent"  (+ start-at 3) (.addInteger -9))
...
(with-field-new rconsumer "min_price_increment_mantissa"  (+ start-at 4) (.addLong src-value))))
		...
		(with-field-new rconsumer "min_price_increment_exponent"  (+ start-at 5) (.addInteger -9))
...
(with-field-new rconsumer "min_price_increment_amount_mantissa"  (+ start-at 6) (.addLong src-value))))
		...
		(with-field-new rconsumer "min_price_increment_amount_exponent"  (+ start-at 7) (.addInteger -9))

(with-field-new rconsumer "display_factor_mantissa"  (+ start-at 8) (.addLong (.mantissa (.displayFactor top-level))))

		(with-field-new rconsumer "display_factor_exponent"  (+ start-at 9) (.addInteger -9))
...
(with-field-new rconsumer "unit_of_measure_qty_mantissa"  (+ start-at 10) (.addLong src-value))))
		...
		(with-field-new rconsumer "unit_of_measure_qty_exponent"  (+ start-at 11) (.addInteger -9))
...
(with-field-new rconsumer "trading_reference_price_mantissa"  (+ start-at 12) (.addLong src-value))))
		...
		(with-field-new rconsumer "trading_reference_price_exponent"  (+ start-at 13) (.addInteger -9))
...
(with-field-new rconsumer "low_limit_price_mantissa"  (+ start-at 14) (.addLong src-value))))
		...
		(with-field-new rconsumer "low_limit_price_exponent"  (+ start-at 15) (.addInteger -9))
...
(with-field-new rconsumer "high_limit_price_mantissa"  (+ start-at 16) (.addLong src-value))))
		...
		(with-field-new rconsumer "high_limit_price_exponent"  (+ start-at 17) (.addInteger -9))


(do (adapt-events MDInstrumentDefinitionOption55Decoder$NoEventsDecoder (+ start-at 18))

(adapt-md-feed-types MDInstrumentDefinitionOption55Decoder$NoMDFeedTypesDecoder (+ start-at 19))

(adapt-inst-attrib MDInstrumentDefinitionOption55Decoder$NoInstAttribDecoder (+ start-at 20))

(adapt-lot-type-rules MDInstrumentDefinitionOption55Decoder$NoLotTypeRulesDecoder (+ start-at 21))

(adapt-underlyings MDInstrumentDefinitionOption55Decoder$NoUnderlyingsDecoder (+ start-at 22))

(adapt-related-instruments MDInstrumentDefinitionOption55Decoder$NoRelatedInstrumentsDecoder (+ start-at 23))
)(adapt-common (+ start-at 24) MDInstrumentDefinitionOption55Decoder)
)  
  (defn ^ParquetRecordWriter make-record-writer [prefix]
   (make-parquet-integrated-writer (str prefix "-md-instrument-definition-option55.parquet")
  ...
  (defn decode [subOffset actingBlockLength actingVersion buffer]
    (.wrap (new MDInstrumentDefinitionOption55Decoder) buffer subOffset actingBlockLength actingVersion))  
