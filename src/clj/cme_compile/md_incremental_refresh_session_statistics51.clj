(ns cme-compile.md-incremental-refresh-session-statistics51
...
(defn- message-write [^RecordConsumer rconsumer #^long start-at ^MDIncrementalRefreshSessionStatistics51Decoder top-level]
  ...
  (let [repetition-decoder (.noMDEntries top-level)]
    (when (> (.count repetition-decoder) 0)
      ...
      (while (.hasNext ^java.util.Iterator repetition-decoder)
        (let [^MDIncrementalRefreshSessionStatistics51Decoder$NoMDEntriesDecoder repetition (.next ^java.util.Iterator repetition-decoder)]
	  ...
	  (with-field-new rconsumer "md_entry_px_exponent" 1 (.addInteger -9))
          ...
          (let [src-value (.mDEntrySize repetition)]
            (when (not= src-value (. MDIncrementalRefreshSessionStatistics51Decoder$NoMDEntriesDecoder mDEntrySizeNullValue))
  ...
  (adapt-common (+ start-at 1) MDIncrementalRefreshSessionStatistics51Decoder)
  )  
(defn ^ParquetRecordWriter make-record-writer [prefix]
  (make-parquet-integrated-writer (str prefix "-md-incremental-refresh-session-statistics51.parquet")
...
(defn decode [subOffset actingBlockLength actingVersion buffer]
  (.wrap (new MDIncrementalRefreshSessionStatistics51Decoder) buffer subOffset actingBlockLength actingVersion))  
