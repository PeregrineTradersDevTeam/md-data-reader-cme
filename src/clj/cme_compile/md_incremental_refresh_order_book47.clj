(ns cme-compile.md-incremental-refresh-order-book47
...
(defn- message-write [^RecordConsumer rconsumer #^long start-at ^MDIncrementalRefreshOrderBook47Decoder top-level]
  ...
  (let [repetition-decoder (.noMDEntries top-level)]
    (when (> (.count repetition-decoder) 0)
      ...
      (while (.hasNext ^java.util.Iterator repetition-decoder)
        (let [^MDIncrementalRefreshOrderBook47Decoder$NoMDEntriesDecoder repetition (.next ^java.util.Iterator repetition-decoder)]

          (.startGroup rconsumer)
          (let [src-value (.orderID repetition)]
            (when (not= src-value (. MDIncrementalRefreshOrderBook47Decoder$NoMDEntriesDecoder orderIDNullValue))
              (with-field-new rconsumer "order_id" 0 (.addLong src-value))))

          (let [src-value (.mDOrderPriority repetition)]
            (when (not= src-value (. MDIncrementalRefreshOrderBook47Decoder$NoMDEntriesDecoder mDOrderPriorityNullValue))
          ...
          (let [src-value (.mDDisplayQty repetition)]
            (when (not= src-value (. MDIncrementalRefreshOrderBook47Decoder$NoMDEntriesDecoder mDDisplayQtyNullValue))
  ...
  (adapt-common (+ start-at 1) MDIncrementalRefreshOrderBook47Decoder))
(defn ^ParquetRecordWriter make-record-writer [prefix]
  (make-parquet-integrated-writer (str prefix "-md-incremental-refresh-order-book47.parquet")
...
(defn decode [subOffset actingBlockLength actingVersion buffer]
  (.wrap (new MDIncrementalRefreshOrderBook47Decoder) buffer subOffset actingBlockLength actingVersion))
