(ns cme-compile.snapshot-full-refresh38
...
(defn- message-write [^RecordConsumer rconsumer #^long start-at ^SnapshotFullRefresh38Decoder top-level]
  ...
  (with-field-new rconsumer "high_limit_price_exponent"  (+ start-at 1) (.addInteger -7))
  ...
  (with-field-new rconsumer "low_limit_price_exponent"  (+ start-at 3) (.addInteger -7))
  ...
  (with-field-new rconsumer "max_price_variation_exponent"  (+ start-at 5) (.addInteger -7))
  ...
  (let [repetition-decoder (.noMDEntries top-level)]
    (when (> (.count repetition-decoder) 0)
      ...
      (while (.hasNext ^java.util.Iterator repetition-decoder)
        (let [^SnapshotFullRefresh38Decoder$NoMDEntriesDecoder repetition (.next ^java.util.Iterator repetition-decoder)]
          ...
          (let [src-value (.mDEntrySize repetition)]
            (when (not= src-value (. SnapshotFullRefresh38Decoder$NoMDEntriesDecoder mDEntrySizeNullValue))
              (with-field-new rconsumer "md_entry_size" 2 (.addInteger src-value))))

          (let [src-value (.numberOfOrders repetition)]
            (when (not= src-value (. SnapshotFullRefresh38Decoder$NoMDEntriesDecoder numberOfOrdersNullValue))
              (with-field-new rconsumer "number_of_orders" 3 (.addInteger src-value))))

          (let [src-value (.mDPriceLevel repetition)]
            (when (not= src-value (. SnapshotFullRefresh38Decoder$NoMDEntriesDecoder mDPriceLevelNullValue))
              (with-field-new rconsumer "md_price_level" 4 (.addInteger src-value))))

          (let [src-value (.tradingReferenceDate repetition)]
            (when (not= src-value (. SnapshotFullRefresh38Decoder$NoMDEntriesDecoder tradingReferenceDateNullValue))
          ...
          (.endGroup rconsumer))) (.endField rconsumer "md_entries" (+ start-at 6))))

  (adapt-common (+ start-at 7) SnapshotFullRefresh38Decoder))
(defn ^ParquetRecordWriter make-record-writer [prefix]
  (make-parquet-integrated-writer (str prefix "-snapshot-full-refresh38.parquet")
...
(defn decode [subOffset actingBlockLength actingVersion buffer]
  (.wrap (new SnapshotFullRefresh38Decoder) buffer subOffset actingBlockLength actingVersion))
