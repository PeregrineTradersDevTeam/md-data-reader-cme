  ...
  (ns cme-compile.snapshot-full-refresh-order-book44
  ...
  (defn- message-write [^RecordConsumer rconsumer #^long start-at ^SnapshotFullRefreshOrderBook44Decoder top-level]
...
(let [^SnapshotFullRefreshOrderBook44Decoder$NoMDEntriesDecoder repetition (.next ^java.util.Iterator repetition-decoder)]
...
(let [src-value (.mDOrderPriority repetition)]
 (when (not= src-value (. SnapshotFullRefreshOrderBook44Decoder$NoMDEntriesDecoder mDOrderPriorityNullValue))
...
(adapt-common (+ start-at 1) SnapshotFullRefreshOrderBook44Decoder)
)  
  (defn ^ParquetRecordWriter make-record-writer [prefix]
   (make-parquet-integrated-writer (str prefix "-snapshot-full-refresh-order-book44.parquet")
  ...
  (defn decode [subOffset actingBlockLength actingVersion buffer]
    (.wrap (new SnapshotFullRefreshOrderBook44Decoder) buffer subOffset actingBlockLength actingVersion))  
