(ns cme-compile.md-instrument-definition-spread
...
(defmacro adapt-common [start-at decoder-type]
  `(do
     ...
     (let [src-value# (.underlyingProduct ~'top-level)]
       (when (not= src-value# (. ~decoder-type underlyingProductNullValue))
         (with-field-new ~'rconsumer "underlying_product"  (+ ~start-at 7) (.addInteger src-value#))))
     (with-field-new ~'rconsumer "security_exchange"  (+ ~start-at 8) (.addBinary (Binary/fromCharSequence (.toString (.securityExchange ~'top-level)))))
     ...
     (with-field-new ~'rconsumer "security_id_source"  (+ ~start-at 13) (.addInteger (int \8)))
     ...
     (with-field-new ~'rconsumer "security_sub_type"  (+ ~start-at 21) (.addBinary (Binary/fromCharSequence (.securitySubType ~'top-level))))

     (with-field-new ~'rconsumer "user_defined_instrument"  (+ ~start-at 22) (.addInteger (int (.userDefinedInstrument ~'top-level))))

     (with-field-new ~'rconsumer "match_algorithm"  (+ ~start-at 23) (.addInteger (.matchAlgorithm ~'top-level)))

     (with-field-new ~'rconsumer "min_trade_vol"  (+ ~start-at 24) (.addInteger (.minTradeVol ~'top-level)))

     (with-field-new ~'rconsumer "max_trade_vol"  (+ ~start-at 25) (.addInteger (.maxTradeVol ~'top-level)))

     (let [src-value# (.priceDisplayFormat ~'top-level)]
       (when (not= src-value# (. ~decoder-type priceDisplayFormatNullValue))
         (with-field-new ~'rconsumer "price_display_format"  (+ ~start-at 26) (.addInteger src-value#))))
     ...
     (let [src-value# (.mainFraction ~'top-level)]
       (when (not= src-value# (. ~decoder-type mainFractionNullValue))
         (with-field-new ~'rconsumer "main_fraction"  (+ ~start-at 32) (.addInteger src-value#))))

     (let [src-value# (.subFraction ~'top-level)]
       (when (not= src-value# (. ~decoder-type subFractionNullValue))
         (with-field-new ~'rconsumer "sub_fraction"  (+ ~start-at 33) (.addInteger src-value#))))

     (let [src-value# (.tradingReferenceDate ~'top-level)]
       (when (not= src-value# (. ~decoder-type tradingReferenceDateNullValue))
         (with-field-new ~'rconsumer "trading_reference_date"  (+ ~start-at 34) (.addInteger src-value#))))
...
(def message-schema "  optional int64 min_price_increment_mantissa;
  ...
  optional int64 price_ratio_mantissa;
  optional int32 price_ratio_exponent;
  ...
  repeated group legs {
    optional int32 leg_security_id;
    optional int32 leg_security_id_source;
    optional int32 leg_side;
    optional int32 leg_ratio_qty;
    optional int64 leg_price_mantissa;
    optional int32 leg_price_exponent;
    optional int32 leg_option_delta_mantissa;
    optional int32 leg_option_delta_exponent;
  }
  ...
  optional binary security_sub_type;
